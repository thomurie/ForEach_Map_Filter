...
function onlyEvenValues(arr) {
  const evens = [];
  arr.forEach((element) => {
    element % 2 === 0 ? evens.push(element) : null;
  });
  return evens;
}
/*
...
function showFirstAndLast(arr) {
  const onlyFistAndLast = [];
  arr.forEach((element) => {
    let combined = "";
    const firstLetter = element[0];
    const lastLetter = element[element.length - 1];
    combined += firstLetter;
    combined += lastLetter;
    onlyFistAndLast.push(combined);
  });
  return onlyFistAndLast;
}
...
function vowelCount(str) {
  const makeStrArray = str.toLowerCase().split("");
  const vowels = ["a", "e", "i", "o", "u"];
  const vowelsInString = {};
  makeStrArray.forEach((element) => {
    if (vowels.indexOf(element) !== -1) {
      if (Boolean(vowelsInString[element]) === false) {
        vowelsInString[element] = 1;
      } else {
...
function find(arr, searchValue) {
  return arr.filter((elem) => elem === searchValue)[0];
}
/*
